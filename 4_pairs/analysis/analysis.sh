...
function save_top_lifetimes(){
  ...
  for chain in "${chains[@]}"; do
    ...
    python3 ${an1}/extract_trajectories_from_dataframe.py -join -sel ${sel} -i ${df} -f ${f} -trj ${trj} -o . -prefix chain_${chain} > chain_${chain}.log 2>&1 &
  done
}
...
function binding_pose_pdb(){
  ...
  for chain in "${chains[@]}"; do
    ...
    f=${an1}/chain_${chain}/initial_fit_merged.pdb
    trj=${an1}/top_lifetimes_pdbs/chain_${chain}_joined.xtc
    python3 ${an1}/binding_pose.py -f ${f} -trj ${trj} -tpr ${an1}/chain_${chain}/protein.tpr -sel1 "${sel1}" -sel2 "${sel2}" -o binding_poses_chain_${chain} -filter 0.8 --cutoff 0.5 0.75 1 1.5 2 >> chain_${chain}_binding.log 2>&1 &
    done
}

function lifetime_analysis_filtered(){
  chains=("4" "r" "s")
  ...
  for chain in "${chains[@]}"; do
    ...
    gro=initial_fit_merged.pdb
    #xtc=test_ultrashort.xtc
    xtc==${an1}/top_lifetimes_pdbs/chain_${chain}_joined.xtc
    sel1="chainID A"
    sel3="chainID ${chain} and not resname *GG* *SQ* *PG* W* HOH *HG*" # Only carotenes, chlorophylls and proteins
    cutoff=8
    dt=2 # time step between frames
    min_event_ns=1000
    echo "Starting contact analysis for chain ${chain}..."
    python3 ${an1}/lifetime_analysis.py -dt ${dt} -min_event_ns ${min_event_ns} -cutoff "${cutoff}" -f ${gro} -traj ${xtc} -sel1 "${sel3}" -sel2 "${sel1}" -o ${odir}/lifetime -prefix filtered_resids_chain_${chain} -group_by1 "resids" -group_by2 "resids" > ${odir}/lifetime/filtered_resids_chain_${chain}.log 2>&1 &
    python3 ${an1}/lifetime_analysis.py -dt ${dt} -min_event_ns ${min_event_ns} -cutoff "${cutoff}" -f ${gro} -traj ${xtc} -sel1 "${sel3}" -sel2 "${sel1}" -o ${odir}/lifetime -prefix filtered_chain_${chain} -group_by1 "chainIDs" -group_by2 "resids" > ${odir}/lifetime/filtered_chain_${chain}.log 2>&1 &
    python3 ${an1}/lifetime_analysis.py -dt ${dt} -min_event_ns ${min_event_ns} -cutoff "${cutoff}" -f ${gro} -traj ${xtc} -sel1 "${sel2}" -sel2 "${sel3}" -o ${odir}/lifetime -prefix filtered_psbs_chain_${chain} -group_by1 "chainIDs" -group_by2 "resids" > ${odir}/lifetime/filtered_psbs_chain_${chain}.log 2>&1 &
  done
}
